      ...
      Console.WriteLine("#define CG_DirectX // render DirectX.");
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            if (input.IsKeyPressed(Keys.D4) && objetoSelecionado != null)
                objetoSelecionado.MatrizRotacaoZBBox(-10);
            if (input.IsKeyPressed(Keys.Enter) && objetoSelecionado != null)
            {
            ...
            {
                objetoSelecionado.PontosRemoverVertice(getSRUPonto(), 0);
            }
            ...
            { //pick
                ...
                {
                    addPontosSubsequentesPoligono();
                }
                else
                {//Se é o primeiro click na tela, não cria polígono e apenas salva o valor do click na lista de pontos.
                    primeiroClick = true;
        ...
        private void addPrimeiroPontoPoligono()
        {
            addPontoColecao();
        }

        private void addPontosSubsequentesPoligono()
        {
            addPontoColecao();
            //Cria o poligono apenas quando desenhar a linha(segundo click)
            if (!criouLinha)
            {
                objetoSelecionado = new Poligono(mundo, ref rotuloNovo, pontosPoligono);
                criouLinha = true;
            }
            else//Se já criou a linha, não cria mais o polígono mas sim adiciona pontos no já existente(terceiro click em diante).
            {
                objetoSelecionado.PontosAdicionar(getSRUPonto());
            }
        }

        private void addPontoColecao()
        {
            this.pontosPoligono.Add(getSRUPonto());
        }

        private Ponto4D getSRUPonto()
        {//Metodo para buscar o ponto de click dentro do plano cartesiano
            int janelaLargura = Size.X;
            int janelaAltura = Size.Y;
            Ponto4D mousePonto = new Ponto4D(MousePosition.X, MousePosition.Y);
            Ponto4D sruPonto = Utilitario.NDC_TelaSRU(janelaLargura, janelaAltura, mousePonto);
